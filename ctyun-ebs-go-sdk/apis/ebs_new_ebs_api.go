...
type EbsNewEbsRequest struct {
	...
	KmsUUID   string `json:"kmsUUID,omitempty"`   /*  如果是加密盘，需要提供kms的uuid，传入空字符串""则使用默认密钥。  */
	...
	DiskSize int32 `json:"diskSize,omitempty"` /*  云硬盘大小，单位GB。数据盘的取值范围：
	...
	CycleCount int32 `json:"cycleCount,omitempty"` /*  包周期数。onDemand为false时必须指定。
	周期为年（year）时，周期最大长度不能超过5年。
	周期为月（month）时，周期最大长度不能超过60月。  */
	ImageID string `json:"imageID,omitempty"` /*  镜像ID，如果用镜像创建，只支持数据盘的私有镜像和共享镜像，所创建的数据盘的所在地域要与镜像源一致，容量不可小于镜像对应的磁盘容量。
	不支持批量创建操作，从镜像创建的数据盘不支持加密、ISCSI和FCSAN高级配置。  */
	AzName          string `json:"azName,omitempty"`          /*  多可用区资源池下，必须指定可用区。  */
	ProvisionedIops int32  `json:"provisionedIops,omitempty"` /*  XSSD类型盘的预配置iops值，最小值为1，其他类型的盘不支持设置。具体取值范围如下：
...
type EbsNewEbsReturnObjResourcesResponse struct {
	...
	Status           int32  `json:"status,omitempty"`           /*  资源状态。  */
	IsMaster         *bool  `json:"isMaster"`                   /*  是否是主资源项。  */
	ItemValue        int32  `json:"itemValue,omitempty"`        /*  资源规格，即为磁盘大小，单位为GB。  */
