...
type EbsListEbsSnapRequest struct {
	...
	SnapshotID      string `json:"snapshotID,omitempty"`      /*  云硬盘快照ID。  */
	SnapshotName    string `json:"snapshotName,omitempty"`    /*  云硬盘快照名称。支持输入名称的一部分做模糊匹配。  */
	SnapshotStatus  string `json:"snapshotStatus,omitempty"`  /*  云硬盘快照状态。取值为：<br>●available：可用。<br>●freezing：冻结。<br>●creating：创建中。<br>●deleting：删除中。<br>●rollbacking：回滚中。<br>●cloning：从快照创建云硬盘中。<br>●error：错误。  */
	SnapshotType    string `json:"snapshotType,omitempty"`    /*  云硬盘快照创建类型。取值为：<br>●manu：手动。<br>●timer：自动。  */
	VolumeAttr      string `json:"volumeAttr,omitempty"`      /*  云硬盘属性。取值为：<br>●data：数据盘。<br>●system：系统盘。  */
	RetentionPolicy string `json:"retentionPolicy,omitempty"` /*  云硬盘快照保留策略。取值为：<br>●forever：永久保留。<br>●custom：自定义保留天数。  */
}
...
type EbsListEbsSnapReturnObjResponse struct {
	SnapshotSize      int32                                          `json:"snapshotSize,omitempty"`      /*  云硬盘快照使用量，单位为B。  */
	SnapshotTotal     int32                                          `json:"snapshotTotal,omitempty"`     /*  云硬盘快照总个数。  */
	SnapshotList      []*EbsListEbsSnapReturnObjSnapshotListResponse `json:"snapshotList"`                /*  云硬盘快照列表。  */
	SnapshotManuTotal int32                                          `json:"snapshotManuTotal,omitempty"` /*  云硬盘手动快照总个数，只有传diskID时返回此字段。  */
}
...
type EbsListEbsSnapReturnObjSnapshotListResponse struct {
	...
	VolumeSize   int32  `json:"volumeSize,omitempty"`   /*  云硬盘大小。  */
